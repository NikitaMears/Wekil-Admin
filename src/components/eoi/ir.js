...
function RFPIR() {
  ...
  const [collapsed, setCollapsed] = useState(true); // Set initial state to collapsed
  ...
  return (
    <>
                ...
                {irData && irData.file && (
                  <>
                    ...
                    {irData.file.endsWith('.pdf') ? (
                      ...
                      <iframe title="Firm Experience" src={`https://kmsbe.frontieri.com/kmsApi/${irData.file}`} style={{ width: "100%", height: "400px" }}></iframe>
      ...
      <Col span={24} md={24} className="mb-24">
        <Collapse bordered={false} activeKey={collapsed ? [] : ['1']} onChange={toggleCollapse}>
          <Panel header={<h6 className="font-semibold m-0">Content</h6>} key="1">
            <Card bordered={false} className="header-solid h-full" bodyStyle={{ paddingTop: 0, paddingBottom: 16 }}>
              <p className="text-dark">{irData && irData.content}</p>
            </Card>
          </Panel>
        </Collapse>
      </Col>

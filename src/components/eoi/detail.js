...
function EOIDetail() {
  ...
  const { id } = useParams();
  const { data: rfpData, loading, postFormData } = useFetchWithToken(`eois/${id}`); // Fetch RFP details using useFetchWithToken hook
  ...
  useEffect(() => {
    if (rfpData) {
      setDocs([{ uri: `https://kmsbe.frontieri.com/kmsApi/${rfpData.file}` }]);
    }
  }, [rfpData]);
  const docs2 = [
  ...
  const handleUpload = async () => {
    ...
    try {
      await postFormData(formData, `uploadEOI/${id}`); // Upload RFP using postFormData function from useFetchWithToken hook
...
<iframe title="No CV" src={`https://kmsbe.frontieri.com/kmsApi/${rfpData.file}`} style={{ width: "100%", height: "400px" }}></iframe>

...
const EditEOIForm = ({ formData, setFormData, closeModal, refetchData }) => {
  const [form] = Form.useForm();
  const history = useHistory();

  const [submitted, setSubmitted] = useState(false);
  const { postData, putData, postFormData } = useFetchWithToken('eois');
  ...
  return (
    <Form form={form} onFinish={onFinish} layout="vertical">
      <Row gutter={16}>
        ...
        <Col span={12}>
          <Form.Item label="EOI Number" name="rfpNo" rules={[{ required: true, message: 'Please enter an RFP Number' }]}>
            <Input />
          </Form.Item>
        </Col>
      </Row>

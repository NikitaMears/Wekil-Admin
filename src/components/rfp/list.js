...
import { Table,Popconfirm, Card, Upload, message, Input, Row, Col,Dropdown, Modal, Button, Checkbox , Tag, Tabs, Spin} from "antd";
...
function RFPList() {
  ...
  const {  postFormData, refetchData, loading, error, token } = useFetchWithToken("rfps");
...
/* eslint-enable no-restricted-globals */
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("https://kmsbe.frontieri.com/kmsApi/rfps",  {  headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
      },});
...
<TabPane tab="Details" key="1">
  ...
  <Button type="link" onClick={() => history.push(`/rfpDetails/${selectedRow.id}`)} style={{ color: 'green' }}>
Details  </Button>
</div>
      </div>
    }
